...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QToolBar, QAction, 
                             ...
                             QGraphicsOpacityEffect, QDialog, QPushButton, QFormLayout)
...
# -------------------------
# Login Dialog (visual + fake auth)
# -------------------------
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Login - Infinity Browser")
        self.setModal(True)
        # Fix size but comfortable
        self.setFixedSize(380, 220)
        self._anim = None

        # estilo dark consistente com o app
        self.setStyleSheet("""
            QDialog {
                background: #121212;
            }
            ...
            QPushButton {
                background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #6a5acd, stop:1 #8a75ed);
                color: #fff;
                border: none;
                padding: 8px 12px;
                border-radius: 6px;
    ...
    def init_ui(self):
        ...
        title = QLabel("<b>Entrar no Infinity</b>")
        ...
        self.pass_input.setEchoMode(QLE.Password)
        ...
        enter_btn.clicked.connect(self.attempt_login)
    ...
    def attempt_login(self):
        """Fake auth: aceita se ambos os campos não estiverem vazios.
           Exibe mensagem especial se for admin/1234."""
        username = self.user_input.text().strip()
        password = self.pass_input.text().strip()

        if not username or not password:
            QMessageBox.warning(self, "Erro", "Preencha Usuário e Senha (é só uma simulação).")
            return

        # Fake check: admin special
        if username == "admin" and password == "1234":
            QMessageBox.information(self, "Bem-vindo", f"Administrador {username} autenticado (simulação).")
        else:
            QMessageBox.information(self, "Bem-vindo", f"Usuário {username} autenticado (simulação).")

        # Faz o fade-out do diálogo e aceita
        try:
...
class MainWindow(QMainWindow):
    ...
    def update_url_bar(self, url):
        ...
        if current_browser and current_browser == self.sender():
            ...
            # Interceptar URLs especiais
            if url_str == "infinity://home":
                self.navigate_home()
            else:
    ...
    def open_login_dialog(self):
        # Fade principal para foco
        fade_out_widget(self, duration=140)
        dlg = LoginDialog(self)
        result = dlg.exec_()
        # Restaurar aparência principal
        QTimer.singleShot(120, lambda: fade_in_widget(self, duration=180))
        if result == QDialog.Accepted:
            # pegar usuário (mentirinha) e atualizar botão
            try:
                username = dlg.user_input.text().strip()
                if username:
                    self.login_btn.setText(f"👤 {username}")
                    self.login_btn.setToolTip(f"Logado como {username}")
            except Exception:
                pass
